...
After interruption use `--resume-report previous.jsonl --report new.jsonl`. This preserves the original cutoff and verified committed projects; never reuse the same report as input and output. Continue the same authorized workbook and scope, with a read-only preflight first.
