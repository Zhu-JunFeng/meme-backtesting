...
test('gates retain exact signal timestamps and fail closed on missing CA',()=>{
 const config={entryAfterSignal:false,exitConfig:{closeAtEnd:false}},inputs=[{symbol:{chain:'sol',ca:'A',pairId:'p'}}];
 ...
 assert.equal(result.entryAfterSignal,true);assert.equal(config.entryAfterSignal,false);
});
