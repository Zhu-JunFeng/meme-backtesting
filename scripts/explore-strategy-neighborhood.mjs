...
export async function explore({chain,interval,output,root=process.cwd(),count=256}){
 ...
 const bases=await Promise.all([chain,chain==='sol'?'robin':'sol'].map(async ch=>(await read(`output/research-stable5-20260921/${ch}-candidate.json`)).strategy));
 const candidates=candidatesAround(bases,count),step=interval==='30s'?30000:60000,warmupBars=1500;
 await mkdir(output,{recursive:true});
 const protocol={scope:'Exploratory optimization on previously inspected windows; NOT blind out-of-sample validation. No production access.',chain,interval,valueType:'mcap',boundaries:prior.boundaries,warmupBars,seed:20260922,candidates,engineVersion:ENGINE_VERSION,archiveSha256:manifest.archive.sha256,createdAt:new Date().toISOString(),selection:'Evaluate fold 0 for all; evaluate folds 1 and 2 only if fold 0 has both returns >=0, drawdown <=10%, normal trades >=20. Rank completed candidates by minimum return over all folds and both accounting views.',target:'Both returns >=5%, max drawdown <=10%, >=20 normal trades, in every fold. No leverage or cost reduction.'};
 ...
 for(const c of candidates){
  const folds=[run(c,0)];runs++;if(foldEligible(folds[0])){folds.push(run(c,1),run(c,2));runs+=2;}
  const trial={id:c.id,folds,complete:folds.length===3,pass:targetReached(folds),worstReturn:Math.min(...folds.map(r=>Math.min(r.accountReturn,r.normalReturn))),config:c.config};trials.push(trial);
 ...
 const leaders=trials.filter(r=>r.complete).sort((a,b)=>Number(b.folds.every(foldEligible))-Number(a.folds.every(foldEligible))||b.worstReturn-a.worstReturn).slice(0,10);
 const stress=[];for(const leader of leaders.slice(0,3)){
  ...
  for(const [name,mutate] of variants){const c=clone(leader);mutate(c.config);const folds=[0,1,2].map(f=>run(c,f));runs+=3;stress.push({id:leader.id,name,folds,pass:targetReached(folds)});await save('stress',stress);}
 }
 const result={chain,interval,valueType:'mcap',scope:protocol.scope,completedAt:new Date().toISOString(),candidates:candidates.length,runs,complete:trials.filter(r=>r.complete).length,passed:trials.filter(r=>r.pass).length,allPositiveWithinRisk:trials.filter(r=>r.complete&&r.folds.every(foldEligible)).length,leaders,stress};await save('results',result);log({stage:'complete',runs,passed:result.passed,best:leaders[0]?.worstReturn});return result;
}

if(process.argv[1]&&pathToFileURL(resolve(process.argv[1])).href===import.meta.url){
 const [chain,interval,output]=process.argv.slice(2);assert(output,'Usage: node scripts/explore-strategy-neighborhood.mjs sol|robin 30s|1m NEW_OUTPUT_DIRECTORY');await explore({chain,interval,output:resolve(output)});
}
