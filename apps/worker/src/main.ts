...
async function shutdown(){if(stopping)return;stopping=true;clearInterval(timer);console.log('保存检查点并停止领取任务');await worker.close();await queue.close();await pool.end();}
