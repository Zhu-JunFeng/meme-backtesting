...
async function drawFib(chart:any,v:number){
 ...
 for(const [name,p] of [['Swing Low',f.low],['Swing High',f.high]] as const)if(loadedTimes.has(p.time))await create([point(p.time,p.value)],'text',`${name} ${displayValue(p.value)}\n${beijingTime(p.time,true)}${p.synthetic?' · 补齐 K 线':''}`,{color:'#176b5b',fontsize:12});
 if(loadedTimes.has(f.confirmed.time))await create([point(f.confirmed.time,f.high.value)],'vertical_line',`Pivot 确认 ${beijingTime(f.confirmed.time,true)}`,{linecolor:'#65766e',linestyle:2,showLabel:true});
...
<p v-if="missingMarkers" class="event-detail" role="status">{{missingMarkers}} 个可见区间事件对应 K 线缺失或尚未加载，未绘制点位；事件列表仍可查看。</p><div v-if="fib" class="measurement"><a-radio-group v-model:value="fibView" size="small"><a-radio-button value="full">完整 Fib</a-radio-button><a-radio-button value="trade">买卖区间</a-radio-button></a-radio-group><a-switch v-model:checked="showFib" size="small" :disabled="fib.status!=='available'" /> 显示 Fib <span>0 = 高点 · 1 = 低点 · 北京时间 UTC+8</span></div><a-alert v-if="error" :message="error" type="warning" show-icon><template #action><a-button @click="mountChart">重新加载</a-button></template></a-alert><div class="measurement"><template v-if="selectedBuy"><strong>{{selectedBuy.event_label || '买入'}} {{displayValue(Number(selectedBuy.price))}}</strong><span>光标{{isMcap?'市值':'价格'}}：{{cursorValue==null?'—':displayValue(cursorValue)}}</span><strong :class="valueTone(movement)">{{movement==null?'移动鼠标查看涨跌幅':signedValue(movement)+'%'}}</strong><span>不含交易成本</span><a-button size="small" @click="clearSelection">取消选择</a-button></template><span v-else>点击买入或加仓标记，再移动鼠标或长按拖动图表，比较光标{{isMcap?'市值':'价格'}}与该买入点的涨跌幅。</span></div><p v-if="selectedEvent" class="event-detail"><a-tooltip :title="'原始数据（时间戳未转换）：'+JSON.stringify(selectedEvent.reason_json)">{{eventSummary(selectedEvent)}} ⓘ</a-tooltip></p><div ref="container" class="tv-chart" /><FibAudit :fib="fib" :is-mcap="isMcap" /></template>
