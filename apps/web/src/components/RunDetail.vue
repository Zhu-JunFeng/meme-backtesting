...
let epoch=0,caEpoch=0,poolEpoch=0,timer:number|undefined;
...
async function openCa(row:any,preserve=false) {
 ...
 if(!preserve)pairId.value=data?.pools?.find((p:any)=>!p.noData)?.pairId ?? data?.pools?.[0]?.pairId;
...
watch(pairId,()=>{eventPage.value=1;tradePage.value=1;loadPool();});
watch([eventPage,tradePage],loadPool);
watch(sort,()=>{page.value=1;loadCas();});
onMounted(()=>{load();timer=window.setInterval(()=>{if(["pending","running"].includes(run.value?.status))load();},5000);});
onBeforeUnmount(()=>{epoch++;caEpoch++;poolEpoch++;window.clearInterval(timer);});
</script>
<template>
 <section class="detail">
  ...
  <template v-if="run">
   <header><h2>{{run.name}}</h2><a-tag>{{({pending:'等待中',running:'运行中',completed:'已完成',failed:'失败',cancelled:'已取消'} as any)[run.status]}}</a-tag><span>{{report?.engineVersion || '历史引擎'}} · {{run.config_json.interval}} · {{run.config_json.valueType==='mcap'?'市值':'价格'}}</span></header>
   ...
   <section v-if="ca" class="ca-detail">
    ...
    <TradingViewChart v-else-if="symbol" :key="symbol" ref="chart" :symbol="symbol" :interval="run.config_json.interval" :run-id="run.id" :start-time="pool.startTime" :end-time="pool.endTime" />
    <h3>事件时间线 · 点击定位</h3>
    ...
    <h3>交易明细</h3>
    <a-table :data-source="trades" row-key="id" size="small" :scroll="{x:850}" :pagination="{current:tradePage,pageSize:20,total:tradeTotal,showSizeChanger:false,onChange:(p:number)=>tradePage=p}" :columns="[{title:'首次入场时间',key:'entry_time'},{title:'平均成本',key:'entry_price'},{title:'数量',key:'quantity'},{title:'退出时间',key:'exit_time'},{title:'净盈亏',key:'net_pnl'},{title:'退出原因',key:'exit_reason'}]">
     <template #bodyCell="{column,record}"><template v-if="column.key.endsWith('_time')">{{record[column.key] ? date(record[column.key]):'未平仓'}}</template><template v-else-if="column.key==='exit_reason'">{{labels[record.exit_reason] || '未平仓'}}</template><template v-else>{{fmt(record[column.key])}}</template></template>
    </a-table>
   </section>
