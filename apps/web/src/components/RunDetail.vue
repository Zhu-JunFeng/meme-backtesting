...
<template>
 <section class="detail">
  ...
  <template v-if="run">
   ...
   <a-alert v-if="!includeEnd && analytics" type="info" show-icon  description="以下为保留订单的累计已实现统计，不含浮动盈亏，不是原账户权益；未改变资金占用或后续成交。被排除订单的全部买卖点位和 Fib 同步隐藏。"><template #message>已排除 {{analytics.excluded.count}} 笔结束平仓，净盈亏 <ResultValue :value="analytics.excluded.netPnl" /></template></a-alert>
