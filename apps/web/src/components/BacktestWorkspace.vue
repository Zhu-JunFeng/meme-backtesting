...
import { beijingTime, beijingInputToIso } from "../time";
...
async function create() {
  ...
  try {
    ...
    const dataset = {cas:selectedCas.value.map(({chain,ca})=>({chain,ca})),interval:form.value.interval,valueType:form.value.valueType,startTime:beijingInputToIso(form.value.startTime),endTime:beijingInputToIso(form.value.endTime),filters:caSelection.snapshot()};
