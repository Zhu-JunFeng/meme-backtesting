...
describe('resumable reference parity',()=>{
 ...
 it('retains volume expansion and deterministic input order',()=>{const c=configuration();c.impulseCondition.requireVolumeExpansion=true;c.impulseCondition.volumeExpansionRatio=.8;const inputs=symbols.map(symbol=>({symbol,candles:candles()}));const r=runBacktest(c,inputs),a=execute(c,[...inputs].reverse(),true);r.report.engineVersion=a.report.engineVersion;expect(a).toEqual(r);});
});
