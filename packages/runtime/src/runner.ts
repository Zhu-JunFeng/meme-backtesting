...
async function writeBatch(c:PoolClient,id:string,b:EngineBatch){
 await insertRows(c,'backtest_signals',['run_id','chain','ca','pair_id','time','price','signal_type','reason_json','quantity'],b.signals.map(s=>[id,s.symbol.chain,s.symbol.ca,s.symbol.pairId,s.time,s.price,s.type,JSON.stringify(s.reason),s.quantity ?? null]));
 await insertRows(c,'backtest_trades',['run_id','chain','ca','pair_id','entry_time','entry_price','quantity','exit_time','exit_price','gross_pnl','fees','slippage_cost','tax_cost','net_pnl','exit_reason','holding_bars','adds_json'],b.trades.map(t=>[id,t.symbol.chain,t.symbol.ca,t.symbol.pairId,t.entryTime,t.entryPrice,t.quantity,t.exitTime ?? null,t.exitPrice ?? null,t.grossPnl ?? null,t.fees,t.slippageCost,t.taxCost,t.netPnl ?? null,t.exitReason ?? null,t.holdingBars ?? null,JSON.stringify(t.adds)]));
