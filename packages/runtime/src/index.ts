...
export async function rerun(pool:Pool,queue:Queue,id:string,requestId:string){
 ...
 else{const r=await c.query("INSERT INTO backtest_runs(name,status,config_json,strategy_template_id,strategy_version_id,dataset_json,parent_run_id,runtime_version,queue_scope,phase) SELECT name || ' · 重新回测','pending',config_json,strategy_template_id,strategy_version_id,dataset_json,id,$2,$3,'freezing' FROM backtest_runs WHERE id=$1 RETURNING id,dispatch_no",[id,RUNTIME_VERSION,queuePrefix()]);if(!r.rowCount)throw new Error('任务不存在');result=r.rows[0];await c.query('INSERT INTO backtest_rerun_requests VALUES($1,$2,$3)',[requestId,id,result.id]);}
